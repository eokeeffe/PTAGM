  ...
  : mGLWindow(ImageRef(1920,1080), "PTAMM")
{
  ...
  mpTracker = new Tracker(mVideoSource.Size(), *mpCamera, mvpMaps, mpMap, *mpMapMaker, this);
  mpMapViewer = new MapViewer(mvpMaps, mpMap, mGLWindow);
  mpARDriver = new ARDriver(*mpCamera, mVideoSource.Size(), mGLWindow, *mpMap, *mpMapViewer, ROSDriver);
...
void System::Run()
{
  while(!mbDone)
  {
	...
	mpARDriver->Reset();
      ...
      //mpTracker->TrackFrame2(input);
      
      if(bDrawMap)
      {
    	mpMapViewer->DrawMap(mpTracker->GetNewPose(),mpARDriver->GetOffSetX(),mpARDriver->GetOffSetY(),mpARDriver->GetOffSetZ());
      }
      if(bDrawAR)
      {
        if( !mpTracker->IsLost() )
