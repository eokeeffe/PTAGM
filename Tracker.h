...
{
public:
  Tracker(CVD::ImageRef irVideoSize, const ATANCamera &c, std::vector<Map*> &maps, Map *m, MapMaker &mm, void* sys);
  ...
  void TrackFrame2(cv::Mat imFrame);
  inline cv::Mat GetCurrentFrame() { return inputVideo;}

  inline SE3<> GetCurrentPose() { return mse3CamFromWorld; }
  inline SE3<> GetNewPose() {return newPose;}
...
protected:
  ...
  void SetMiddlePose(SO3<> r, Vector<3> t);
