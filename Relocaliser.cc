...
bool Relocaliser::AttemptRecovery(Map & currentMap, KeyFrame &kCurrent)
{
  ...
  //Get the maps to be iterated. Not the whole set of maps but most likely set of maps.
  /*mapsToLook.clear();
  mapsToLook.push_back(GetMapByID(currentMap.MapID()));
  if(currentMap.MapBefore != currentMap.Successor)
  {
	  mapsToLook.push_back(GetMapByID(currentMap.Successor));
  }
  if(currentMap.MapBefore != currentMap.Predecessor)
  {
	  mapsToLook.push_back(GetMapByID(currentMap.Predecessor));
  }
  mapsToLook.push_back(GetMapByID(currentMap.MapBefore));
  for(int i=0;i<mapsToLook.size();i++)
  {
	  if(mapsToLook[i]!=-1)
	  {
		  ScoreKFs(mvpMaps[mapsToLook[i]], kCurrent);
		  mbNewRun = false;
	  }
  }*/
...
//Eranda Lakshantha
//returns the position of the specified map
int Relocaliser::GetMapByID(int id)
{
	for(int k=0;k<mvpMaps.size();k++)
	{
		if(mvpMaps[k]->MapID() == id)
		{ return k;}
	}
	return -1;
}
